...
MainWindow::MainWindow(std::shared_ptr<RosNode> node, QWidget *parent)
    ...
    auto renderer = vtkSmartPointer<vtkGenericOpenGLRenderWindow>::New();
    vtk_widget_->setRenderWindow(renderer);
    visualizer_.reset(new pcl::visualization::PCLVisualizer(renderer, false));
...
void MainWindow::toggleMapping(bool checked) {
    is_mapping_ = checked;
    if (!checked) mapper_.clear(); // Optional: clear on stop
}

void MainWindow::saveMap() {
    QString filename = QFileDialog::getSaveFileName(this, "Save Map", "", "PCD Files (*.pcd)");
    if (!filename.isEmpty()) {
...
void MainWindow::updateDashboard(double fitness, size_t points) {
    ...
    lbl_fitness_->setText(QString("Overlap Fitness: <span style='color:%1'>%2</span>")
                         .arg(color).arg(fitness, 0, 'f', 4));
